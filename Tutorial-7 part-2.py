...
# sometimes  we would want to use a class  written in some other python file, what do u do?
# you put both the files, which has the class you need and in file you need that class at same level in directory, and then you  just import that file and use that as a module, we have already did that ? right! haha
# btw, don't foget to use docstrings(yep those three quotes comment) at the start of the module file.
# This chapter is ovver, it now really is all about the importing classes from modules,writing multiple classes into one module and importing multiple  classes  from a module, and then importing all the classes of a module and importing an entire module, then imprting a module into a module. all this behaviours aren't something that python forbids you, you are alllowed to do such at your own discretion as how you see the situation fit, in order to manage your code more effectively.
#<-------------------end of document------------>