...
2. It streams the encoded Line Feed character. The `print()` function itself doesn't break the line; the terminal emulator does. When the terminal receives this byte, it physically moves the cursor down to the next line and resets it to the left edge.
...
> *Note: There is one subtle quirk with raw strings—they cannot end with an odd number of backslash characters (e.g., `r"\"` will cause a syntax error). Why? Because even in a raw string, a backslash escapes the quote mark immediately following it, which prevents the string from properly closing.*
...
* `copy.replace(obj, **changes)`: *(New in Python 3.13)* Duplicates the object but swaps out specific fields using keyword arguments (e.g., `copy.replace(my_obj, name="Alice", age=30)`).
...
* **Shallow Copy (`copy.copy`)**: Builds a brand new outer cardboard box, but places shortcuts (references) to the *original* inner shoeboxes inside it. Changing an item inside a shared inner list alters both the original and the copy.
...
* `__replace__(self, /, **changes)`: Defines what happens during `copy.replace()`. Generate a fresh version of your object, intelligently swapping out the attributes provided in the `**changes` keyword arguments (commonly used in dataclasses and namedtuples).
